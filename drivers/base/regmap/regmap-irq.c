...
int devm_regmap_add_irq_chip(struct device *dev, struct regmap *map, int irq,
			     ...
			     struct regmap_irq_chip_data **data)
{
	return devm_regmap_add_irq_chip_fwnode_named(dev, dev_fwnode(map->dev),
						     map, irq, irq_flags,
						     irq_base, chip, data, NULL);
}
EXPORT_SYMBOL_GPL(devm_regmap_add_irq_chip);

/**
 * devm_regmap_add_irq_chip_named() - Resource managed regmap_add_irq_chip()
 *
 * @dev: The device pointer on which irq_chip belongs to.
 * @map: The regmap for the device.
 * @irq: The IRQ the device uses to signal interrupts
 ...
 * unbound.
 */
int devm_regmap_add_irq_chip_named(struct device *dev, struct regmap *map,
...
{
	return devm_regmap_add_irq_chip_fwnode_named(dev, dev_fwnode(map->dev),
						map, irq, irq_flags, irq_base,
						chip, data, domain_suffix);
}
EXPORT_SYMBOL_GPL(devm_regmap_add_irq_chip_named);
