...
static int test_get_soc_by_ocv(struct sw_gauge *sw __attribute__((unused)),
			       int ocv, int temp __attribute__((unused)),
			       int *soc);
static int test_get_soc_by_ocv(struct sw_gauge *sw, int ocv, int temp, int *soc)
{
	int i = 0;
...
static int test_get_ocv_by_soc(struct sw_gauge *sw, int dsoc, int temp, int *ocv)
{
	int i = 0;
...
static bool test_is_relaxed(struct sw_gauge *gauge, int *rex_volt)
{
	/* TODO: Add stub for testing relax voltage set CC stuff */
	return 0;
}

static int test_get_temp(struct sw_gauge *gauge, int *temp)
{
	*temp = TEST_TEMP;
...
//static int test_uah[] = {};
static int test_get_uah(struct sw_gauge *gauge, int *uah)
{
	static int iter = 0;
...
static int test_get_uah_from_full(struct sw_gauge *gauge, int *uah)
{
...
static int test_update_cc_uah(struct sw_gauge *gauge, int bcap)
{
	pr_err("Should not be here!\n");
...
static int test_get_vsys(struct sw_gauge *gauge, int *uv)
{
	static int iter = 0;
...
//		msleep(70); /* delay until iteration is completed */
		...
		if (ret) {
			...
			pr_info("i=%d SOC=%u FULL=%u DESIGN=%u NOW=%u cyc=%u\n",
				 i, soc.intval, chg.intval, chg_des.intval,
				 chg_now.intval, cyc.intval);
		}
		TEST_loop = 0;
		LOST_teep = 1;
		wake_up(&_SWG_TESTwq);
