				      ...
				      struct power_supply_battery_info *info)
{
	...
	info->temp_dgrd_table_size = 0;
	info->temp_dgrd_table = NULL;
	...
	err = get_fwnode_tuple_array(dev, battery_node,
				     "temp-degrade-table",
				     (struct psy_int_tuple **)&info->temp_dgrd_table,
				     &info->temp_dgrd_table_size);
	if (err == -ENOMEM) {
				...
				int table_len, int dcap)
{
	...
	if (i > 0 && i < table_len)
		ocv = fixp_linear_interpolate(table[i].capacity * 10,
					      table[i].ocv,
				...
				int table_len, int ocv)
{
	...
	if (i > 0 && i < table_len)
		dcap = fixp_linear_interpolate(table[i].ocv,
					      table[i].capacity * 10,
					      table[i - 1].ocv,
					      table[i - 1].capacity * 10,
					      ocv);
	else if (i == 0)
		dcap = table[0].capacity * 10;
	else
		dcap = table[table_len - 1].capacity * 10;
...
int power_supply_ocv2cap_simple(struct power_supply_battery_ocv_table *table,
				int table_len, int ocv)
{
	return power_supply_ocv2dcap_simple(table, table_len, ocv) / 10;
}
				 ...
				 int ocv, int temp)
{
	...
	ret = power_supply_batinfo_ocv2dcap(info, ocv, temp);
	if (ret != -EINVAL)
		ret /= 10;
