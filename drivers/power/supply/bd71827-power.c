...
static int bd71827_voltage_to_capacity(struct simple_gauge *sw
...
/*
static irqreturn_t bd71815_chg_state_changed(int irq, void *data)
{

	simple_gauge_run(pwr->sw);
	power_supply_changed(pwr->ac);
	dev_dbg(pwr->dev, "CHG state changed\n");
...
static irqreturn_t bd718x7_chg_done(int irq, void *data)
{
	...
	/* Battery is likely to be FULL => run simple_gauge to initiate CC setting */
	simple_gauge_run(pwr->sw);
