				    ...
				    struct device_node *np)
{
	...
	if (data->num_ranges) {
		...
		if ((val & data->desc.enable_mask) != data->desc.enable_mask) {
			dev_warn(dev,
				 "%s: enabled. Can't set inital voltage\n",
				 data->desc.name);
			goto get_initial;
		}
				  ...
				  struct regulator_dev *rdev)
{
	int i;
	void *retp;
	static const char *single_out_errb_irqs[] = {
				    ...
				    struct regulator_dev **rdev, int num_rdev)
{
	int i, num_irqs;
	void *retp;
	static const char *global_errb_irqs[] = {
...
static int bd96801_probe(struct platform_device *pdev)
{
	...
	return bd96801_global_errb_irqs(pdev, all_rdevs, ARRAY_SIZE(all_rdevs));
}
