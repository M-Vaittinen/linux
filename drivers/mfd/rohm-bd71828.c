...
static const struct resource bd71851_rtc_irqs[] = {
	DEFINE_RES_IRQ_NAMED(BD71851_INT_RTC0, "bd70528-rtc-alm-0"),
	DEFINE_RES_IRQ_NAMED(BD71851_INT_RTC1, "bd70528-rtc-alm-1"),
	DEFINE_RES_IRQ_NAMED(BD71851_INT_RTC2, "bd70528-rtc-alm-2"),
};
...
static const struct regmap_irq_chip bd71851_irq_chip = {
	.name = "bd71851_irq",
...
static int bd71828_i2c_probe(struct i2c_client *i2c)
{
	...
	/*
	 * BD71851 has an enable register also for the 'main' level. The IRQ
	 * controller code does not support enabling/disabling 'blocks of
	 * IRQs', so we need to do it here. IRQs will be masked/unmasked by the
	 * IRQ controller code using the IRQ specific enable registers, so we
	 * can just leave them enabled at main level.
	 */
	if (chip_type == ROHM_CHIP_TYPE_BD71851) {
		ret = regmap_set_bits(regmap, BD71851_REG_INT_MAIN_EN,
				      BD71851_INT_MAIN_EN_ALL);
		if (ret)
			return dev_err_probe(&i2c->dev, ret,
					     "Failed to enable IRQ blocks\n");
	}
