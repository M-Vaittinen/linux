...
static int bd96801_i2c_probe(struct i2c_client *i2c)
{
	...
	regulator_res = kzalloc(sizeof(*regulator_res) *
				num_regu_irqs, GFP_KERNEL);
	if (!regulator_res)
		return -ENOMEM;
	...
	ret = devm_regmap_add_irq_chip(&i2c->dev, bd96801->regmap,
				       intb_irq, IRQF_ONESHOT, 0,
				       &bd96801_irq_chip_intb, &intb_irq_data);
	if (ret) {
		dev_err(&i2c->dev, "Failed to add INTB irq_chip\n");
		kfree(regulator_res);
		return ret;
	}
	...
	if (num_errb) {
		ret = devm_regmap_add_irq_chip(&i2c->dev, bd96801->regmap,
					       errb_irq, IRQF_ONESHOT, 0,
					       &bd96801_irq_chip_errb,
					       &errb_irq_data);
		if (ret) {
			dev_err(&i2c->dev, "Failed to add ERRB (%d) irq_chip\n",
				errb_irq);
			return ret;
		}
